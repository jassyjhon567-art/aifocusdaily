...
# ---------------------------------------------------------------------------
def parse_json_object_loose(raw_text):
    """
    Parse a JSON object out of Gemini's text output, tolerating the messy
    edge cases seen in practice even with responseMimeType=application/json:
      - trailing junk after a complete object ("Extra data" errors)
      - an extra/duplicated object concatenated after the first one
      - literal control characters inside string values
    Strategy: try a straight parse first (fast path). If that fails, scan
    for the first '{' and walk forward counting brace depth (respecting
    quoted strings and escapes) to find the matching close brace, then
    parse only that slice. Raises the original-style exception if nothing
    parseable can be found.
    """
    try:
    ...
    # Fallback: allow literal control characters inside strings, in case
    # the model emitted raw newlines instead of escaped \n.
    try:
    ...
    if end == -1:
        raise ValueError("Unbalanced JSON object in Gemini response")
...
"""
    ...
    payload = {
        ...
        "generationConfig": {
            ...
            "temperature": 0.7,
            # A short cap here truncates mid-string on longer bilingual
            # articles, which produces exactly the "Extra data" /
            # "Expecting ',' delimiter" JSON errors we saw in practice.
    ...
    try:
        ...
        if finish_reason == "MAX_TOKENS":
            print("  [gemini] response was truncated (MAX_TOKENS) - treating as failure.")
            return None
...
def generate_post_html(
    ...
    # --- SEO fields ---
    canonical_url = f"{SITE_BASE_URL}/{POSTS_DIR}/{lang}/{slug}.html"
    img_abs_url = f"{SITE_BASE_URL}/{img_path}" if img_path else fallback_img
    og_locale = "bn_BD" if lang == "bn" else "en_US"

    # Attribute-safe (quote-escaped) copies for use inside meta content="".
    title_attr = html.escape(title, quote=True)
    summary_attr = html.escape(summary, quote=True)

    # A lightweight, non-fabricated keyword list: the niche plus the
    # significant words already present in the (human/AI-written) title,
    # rather than inventing search terms unrelated to the actual content.
    title_words = re.findall(r"[A-Za-z\u0980-\u09FF']{3,}", title)
    seen = []
...
def submit_to_google_indexing_with_retry(target_url, max_retries=3, delay_seconds=2):
    ...
    # 401/403 mean the service account isn't authorized/verified for this
    # property in Search Console - retrying won't change that outcome, so
    # we fail fast instead of burning 3 attempts (and 3x the log noise) on
    # every single article.
    NON_TRANSIENT_STATUSES = {401, 403}
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if resp.status_code == 200:
                ...
                return True
            if resp.status_code in NON_TRANSIENT_STATUSES:
                print(
                    f"  [indexing] skipped ({resp.status_code} - service account not "
                    f"verified as owner in Google Search Console for this site). "
                    f"Verify {SITE_BASE_URL} in Search Console and add this service "
                    f"account as a verified owner to enable indexing submissions."
                )
                return False
            print(
...
# ---------------------------------------------------------------------------
def sync_index_html_seo():
    """
    Keep the homepage's canonical/OG/Twitter/JSON-LD block pointed at the
    real SITE_BASE_URL automatically, so nobody has to hand-edit index.html
    after setting the SITE_BASE_URL secret. Replaces everything between the
    SEO_META_START/END markers in index.html; no-op if the file or markers
    aren't present.
    """
    ...
    start_marker = "<!-- SEO_META_START (auto-synced by main.py on every run - do not hand-edit) -->"
    end_marker = "<!-- SEO_META_END -->"
    ...
    new_block = "\n".join([
        ...
        f'<meta property="og:description" content="{html.escape(site_desc, quote=True)}">',
    ...
    if updated != html_content:
        with open(index_path, "w", encoding="utf-8") as f:
            f.write(updated)
        print("  [seo] synced index.html canonical/OG/JSON-LD to current SITE_BASE_URL.")
...
def generate_sitemap():
    """
    Build sitemap.xml by scanning every HTML file actually on disk under
    posts/en and posts/bn - not just the latest 50 in news.json - so older
    articles that have rolled off the homepage feed stay discoverable and
    indexed by search engines.
    """
    urls = [(f"{SITE_BASE_URL}/index.html", None)]
    for lang in ("en", "bn"):
        ...
        for fname in sorted(os.listdir(lang_dir)):
            ...
            urls.append((f"{SITE_BASE_URL}/{lang_dir}/{fname}", mtime))
    ...
    for url, mtime in urls:
        ...
        if mtime:
            lines.append(f"    <lastmod>{mtime.strftime('%Y-%m-%d')}</lastmod>")
        lines.append("  </url>")
    lines.append("</urlset>")

    with open("sitemap.xml", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print(f"  [sitemap] wrote sitemap.xml with {len(urls)} URLs.")
...
def main():
    ...
    # Regenerated every run (cheap, and keeps lastmod dates accurate even
    # when no new articles were published) so search engines always have
    # an up-to-date map of every post that exists on disk.
    generate_sitemap()
    generate_robots_txt()
